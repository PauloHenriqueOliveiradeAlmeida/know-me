...
public class FindPersonWithSameInterestsUseCase {
    ...
    public FindPersonWithSameInterestsUseCase(PersonRepository personRepository) {
    ...
    public List<PersonOutput> execute(ID personId) {
        ...
        List<Person> persons = personRepository.findpersonWithSameInterests(personId);
        return persons.stream().map(
                p -> new PersonOutput(
                    p.id,
                    p.name,
                    p.getInterests(),
                    p.getFriends().stream().map(friendPerson -> friendPerson.id).collect(Collectors.toSet())
                )
        ).toList();
    }
}
